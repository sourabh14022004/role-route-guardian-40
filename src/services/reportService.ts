...
// New function to fetch reports within a date range
export async function fetchReportsByDateRange(startDate?: Date, endDate?: Date, limit = 100) {
  try {
    ...
    if (startDate) {
      ...
      query = query.gte('visit_date', formattedStartDate);
    }
    ...
    if (endDate) {
      ...
      query = query.lt('visit_date', formattedEndDate);
    }
